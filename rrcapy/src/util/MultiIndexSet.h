...
//
#ifndef RRCA_UTIL_MULTIINDEXSET_H_
#define RRCA_UTIL_MULTIINDEXSET_H_
...
struct IndexSetCriterion<TensorProduct> {
  ...
  bool operator()(const T &index) {
    double max = 0;
    for (auto i : index) max = max > double(i) ? max : double(i);
 ...
 **/
template <class Index, IndexSetType T = TotalDegree>
class MultiIndexSet {
 public:
  MultiIndexSet(){};
