...
// information.                                                               //
////////////////////////////////////////////////////////////////////////////////
#ifndef RRCA_DISTRIBUTIONEMBEDDING_JOINTDISTRIBUTIONPOLYNOMIALEMBEDDING_H_
#define RRCA_DISTRIBUTIONEMBEDDING_JOINTDISTRIBUTIONPOLYNOMIALEMBEDDING_H_
...
/*
*    \brief specializes joint ditribtion embedding to polynomials. Here g=h
*/
template<unsigned int order>
class JointPolynomialDistributionEmbedding {
public:
    JointPolynomialDistributionEmbedding ( const Matrix& xdata_, const Matrix& ydata_) :
        xdata ( xdata_ ),
        ydata ( ydata_ ),
        xbasisdim ( binomialCoefficient ( xdata_.rows()+order,xdata_.rows() ) ),
...
//      compute the V matrices
    ...
    ~JointPolynomialDistributionEmbedding(){
        if(modelHasBeenSet){
...
//          compute basisvector
        ...
        const double n(Xs.cols());
         ...
         Matrix oas =  -  VVy_t.transpose() * ygramL * H * xgramL * VVx_t ;
         oas.diagonal() += Vector::Constant(Ys.cols(),Ys.cols());
        
        return((oas.transpose() * oas).trace()/(n*n));
    }
...
//      now compute the kernel matrix block
        ...
        const Matrix res = Qy * H * ( xgramL * newVandermonde_t );
...
//          compute basisvector
        ...
        Matrix inter = H * ( xgramL * newVandermonde_t );
        const RowVector norma = (Qy.colwise().sum() * inter);
        inter.array().rowwise() /= norma.array();
        const Matrix oida = Ys   * Qy; // should be small
...
// // // // // normalization // // // // // // // // 
            ...
            M->constraint ( M_Expr::dot ( LXmeanswrap,M_Expr::mul ( HH_t,LYmeanswrap ) ), M_Domain::equalsTo ( 1.0 ) );
